# Please write a module that gives where behavior to an array of hashes. 
  ...
  def where(hash={})
    ...
    self.each do |n|
      ...
      hash.each do |key, value|
        ...
        elsif key === :quote || key === :title
          if value.match(n[key])
            criteriaMatch << n
          end
        end
      end

      if hash.length == criteriaMatch.length
        matches << criteriaMatch[0]
      end
    end
